(function() {
    ...
    const simulationBtn_80 = document.querySelector('.simulation-btn-80');
    ...
    function loop() {
        ...
        if (recoveredCount < 50) {
          ...
          for(let j=0; j<stopCount; j++) {
            stop_ball = stop_balls[i];

            if (stop_ball.x > canvas.width - radius || stop_ball.x < radius) {
                stop_ball.angle = 180 - stop_ball.angle;
            } else if (stop_ball.y > canvas.height - radius || stop_ball.y < radius) {
                stop_ball.angle = 360 - stop_ball.angle;
            }

            stop_ball.x += Math.cos(toRadian(stop_ball.angle)) * speed;
            stop_ball.y += Math.sin(toRadian(stop_ball.angle)) * speed;
            stop_balls[i].draw();

            stop_ball.nextX = stop_ball.x + Math.cos(toRadian(stop_ball.angle)) * speed;
            stop_ball.nextY = stop_ball.y + Math.sin(toRadian(stop_ball.angle)) * speed;
          }
        }
